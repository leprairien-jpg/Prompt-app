...
# --- FONCTIONS DE SAUVEGARDE (20 PROMPTS) ---
def charger_memoire():
...
def sauver_prompt(p):
    h = charger_memoire()
    h.insert(0, p)
    with open("mes_prompts.json", "w") as f: json.dump(h[:20], f)
    ...
    try:
        ...
        if st.button("✨ Générer le Prompt 5 Étoiles"):
            if not user_input:
                ...
                # SAUVEGARDE (Après la boucle pour ne pas polluer l'IA)
        ...
        # --- BIBLIOTHÈQUE ---
        st.markdown("---")
        with st.expander("📚 Bibliothèque (20 derniers)"):
            for p in charger_memoire():
